...
public class RestaurantDAO implements RestaurantDAO_Interface {
	...
	private static final String GET_restName = "SELECT restName FROM REST";
	private static final String GET_restNo = "SELECT RESTNO FROM REST WHERE restName= ? ";
	@Override
	...
	public ArrayList<String> getRestName() {
		...
		try {
			conn = ds.getConnection();
			pstmt = conn.prepareStatement(GET_restName);
			rs = pstmt.executeQuery();
			while(rs.next()){
				String restuaramtName =rs.getString("restNAME");
				restList.add(restuaramtName);
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public Integer getRestNo(String restuarantSelected) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		Restaurant rest = null;
		...
		try {
			conn = ds.getConnection();
			pstmt = conn.prepareStatement(GET_restNo);
			pstmt.setString(1, restuarantSelected);
			rs = pstmt.executeQuery();
			while(rs.next()){
				restNo=(Integer)rs.getInt("RESTNO");
